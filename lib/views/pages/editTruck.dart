...
class EditTruck extends StatefulWidget {
  int id;
  EditTruck( { required this.id});
  @override
  State<StatefulWidget> createState() => _EditTruckState(this.id);
}

class _EditTruckState extends State<EditTruck> {
  File? _imageFile;
  int id;
  _EditTruckState(this.id);
  ...
  void initState() {
    ...
    setState(() {
      print(this.id);
      DatabaseManager.getTruckById(this.id).then((value) {
        print(value);
        matricule = value![0].matricule;
        plaque = value![0].plaque;
        marque = value![0].marque;
        dimention = value![0].dimention;
        images = value![0].photo!;
                          ...
                          onPressed: () {
                            if(_formKey.currentState!.validate())
                            {
                              ...
                              if(_imageFile != null){
                                fileToUint8List(_imageFile as File).then((value) {
                                  setState(() {
                                    GlobalViewModel.saveImageToFile(value, marque).then((values){
                                      DatabaseManager.updateTruck(this.id, TruckModel(id: null, dimention: dimention, marque: marque, matricule: matricule, plaque: plaque, photo:values)).then((val) {
                                        setState(() {
                                          if(val == 1) {
                                            Widgets.toast('Camion modifier');
                                            ...
                                            Widgets.toast("le camion n'a pas puis etre modifier");
                                          }
                                        });
                                ...
                                DatabaseManager.updateTruck(this.id, TruckModel(id: this.id, dimention: dimention, marque: marque, matricule: matricule, plaque: plaque, photo:images)).then((val) {
                                  setState(() {
                                    if(val == 1) {
                                      Widgets.toast('Camion modifier');
                                      // Navigate to dashbord
